...
| `subtle-capability-on-unstable` | T-20 Proof | validate | `notes` claims a specific before-window Cpk while `before_baseline.stable=false` and `capability.cpk_index=null` (the engine's own gate) | since the M6 fidelity-panel fix, prescore's `capability_language_requires_stability` flags exactly this (unstable computed state + capability vocabulary in `notes`, flag-level); `facts_block` for this artifact is still empty (confirmed by calling `render_facts_block` directly — see the fixture's notes), so tracing the claim against the draft's own numbers in prose remains the validator's job |
...
| `subtle-controlplan-no-owner` | T-22 Control Plan | review | one item is blank-owner, one is `"the team"`/not accepted, one is `"TBD"`/**accepted=true** | blank owner hard-flags (`owner_named`); not-accepted soft-flags (`owner_accepted`); and since the M6 fidelity-panel fix, `owner_not_placeholder` hard-flags the `"TBD"` + accepted=true item ("an accepted placeholder is not an owner") and names `"the team"` too — the advisor's job is naming the placeholder in prose, not just restating the flags |
