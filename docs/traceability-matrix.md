...
| T-35 | Full multi-operator Gage R&R — **shipped early 2026-08-11**; still v2 tier, so the Tier-A count of 25 is unchanged | Measure | v2 |
...
| III.E Measurement system analysis (GR&R, correlation, bias, linearity, percent agreement, P/T) | T-12 (narrow and named honestly: resolution pre-check, then single-operator test/retest **repeatability%** — not "%EV" and not "GRR," per Belt-panel round 2: EV names a component of a full variance-decomposed study, and reproducibility is absent from this check by design — defined in §4a, denominator named as which it is on the output; two-rater attribute agreement reported as % agreement **+ kappa** so chance agreement can't flatter a low-defect process); full multi-operator GR&R = T-35, **shipped 2026-08-11** (two-way crossed ANOVA with interaction; variance components from expected mean squares, negative components clamped and reported as clamped; interaction pooled at alpha = 0.25 with the decision printed; ndc reported beside %GRR; both bases named); bias/linearity explain-only + EXIT-03. Verdict thresholds frozen in §4a | NIST/SEMATECH §2.4 (gauge studies) for structure; generic industry threshold conventions (§4a) for verdicts; NIST ref test | R-MEA-07 | G-msa-01, G-msa-02 (fail path) |
| III.F.1 Process performance vs. process specifications (natural limits vs spec limits) | T-13 (spec limits + operational definition enforced first; one-sided specs supported — Cpk/Ppk computed on the bounded side only and labeled one-sided, Cp/Pp not reported without both limits — Belt-panel round 2) | NIST/SEMATECH §6.1.6 | R-MEA-08 | G-baseline-01 |
...
| VI.A.3 Control charts (X̄-R, X̄-s, ImR/XmR, median, p, np, c, u) | T-21 (I-MR, p — the two a GB reaches for); T-29 (v1.1: X̄-R, np, c, u); X̄-s + median explain-only; selector by data type is a printed decision tree whose first attribute question is **defectives or defects?** — pass/fail units → p; counts per unit/area → EXIT-11 by name (c/u at v1.1), never a p-chart (Belt-panel review) | Standard published constant tables (d2, A2, D3, D4…); NIST/SEMATECH §6.3.2 (variables), §6.3.3 (attributes); NIST ref tests | R-CTL-01 | G-imr-01, G-pchart-01 |
...
| EXIT-03 | Measurement question exceeds the narrow check (bias/linearity/stability study needed) | Names the needed study and states the tool doesn't run it. **Narrowed 2026-08-11:** multi-operator reproducibility is no longer an exit — T-35 shipped and the route names it | Multi-operator GR&R → T-35, in-app. Bias/linearity/stability → human quality engineer / certified Belt |
...
| EXIT-11 | Rates with exposure, or defects counted per unit/area (Poisson-type — includes "scratches per panel," "errors per batch") | Named as count/rate data the v1 battery and chart set don't carry honestly; **p-chart and proportion tests are barred for it by name** (defectives ≠ defects) | c/u chart family (T-29, v1.1) for monitoring; human expert for rate comparisons; DPMO/yield (T-10) remains available as the descriptive summary. A project whose PRIMARY metric is count/rate data lands here at baseline — the honest-exit cascade applies (rubric §1): the project pauses for v1.1 or an expert rather than faking a p-chart baseline |
...
| EXIT-05 | Advisory normality concern (visual + Anderson–Darling p < 0.05, n-aware framing, never silent). Non-normal path, method defined precisely (Belt-panel round 2 — an undefined "percentile method" would print nonstandard numbers under a standard label): when n ≥ 100, **percentile-method performance indices** — Pp\_perc = (USL−LSL)/(p₉₉.₈₆₅−p₀.₁₃₅), Ppk\_perc = min[(USL−p₅₀)/(p₉₉.₈₆₅−p₅₀), (p₅₀−LSL)/(p₅₀−p₀.₁₃₅)] with empirical quantiles (linear interpolation), always labeled "percentile method — not normal-theory Ppk"; below 100 no **percentile** indices. Clarified at M2 build (the terse wording allowed a stricter misreading): a normality concern **never suppresses the normal-theory indices** — they always render with the caveat attached ("caveat path, not a stop"); the percentile method is the gated *addition*, and below its n-floor the caveat rides the normal-theory numbers plus observed yield/DPMO. Fitted-percentile methods (Clements-style) deliberately deferred: they mislead at exactly the sample sizes untrained users have |
| EXIT-06 | Route floors — **the suite's minimum n to emit a test at all** (refuse-to-compute gates for untrained users, explicitly NOT a powered-study guarantee; a test at the floor can be badly underpowered — power and adequacy are handled by R-MEA-05's sample-size guidance going in and R-ANA-05's effect-size/CI discipline coming out; Belt-panel correction of the earlier "cannot reach α" wording, which was false for several routes): Welch / 1-sample t: n ≥ 8 per sample · paired t: ≥ 8 pairs (both raised from 5/6 at Belt-panel round 2 — n=5 invites garbage-as-proof) · one-way ANOVA: ≥ 3 groups and ≥ 4 per group · 1- and 2-proportion: n·p̂ ≥ 5 and n·(1−p̂) ≥ 5 per sample · Mann-Whitney: ≥ 4 per group · Wilcoxon signed-rank: ≥ 6 non-zero differences |
...
| EXIT-09 | Lag-1 autocorrelation both significant (|r₁| > 2/√n) and material (|r₁| ≥ 0.3 — lowered from 0.5 at Belt-panel round 2: moderate dependence already distorts I-MR limits and test error rates) |
| EXIT-10 | Pilot plan declares more than one change |
| EXIT-11 | Data-type field = events over an exposure window (rate), **or defects counted per unit/area** (Poisson-family, c/u-chart territory — Belt-panel addition: defectives are pass/fail units, defects are counts *on* units; the p-chart and proportion tests fit only the first, and count data is refused by name, never forced through them) |
| EXIT-12 | More than one primary comparison declared, or tests run exceed comparisons declared |
| EXIT-13 | ANOVA p < α with ≥ 3 groups **and the user's question asks which groups differ** (Belt-panel round 2, GPT correction: when the pre-declared question was only "does any group differ," the omnibus answer is complete — no exit fires, the advisor just notes pairwise ships v1.1) |
