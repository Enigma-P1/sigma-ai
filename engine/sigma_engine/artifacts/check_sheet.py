...
def check_sheet_export_rows(artifact: CheckSheetArtifact) -> tuple[list[str], list[dict[str, str]]]:
    """(header, rows) for `to_dataset`: one row per entry, `category` as
    the human-readable label (unique, enforced above) so Pareto's category
    column needs no id -> label join downstream. Sorted by timestamp so
    the exported dataset reads as an event log, not tap-entry order."""
    ...
    rows = [
        ...
        for e in sorted(artifact.entries, key=lambda e: e.timestamp)
    ]
    return header, rows
