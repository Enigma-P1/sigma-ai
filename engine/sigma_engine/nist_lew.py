"""NIST StRD reference dataset used for the packaging-spike smoke check.

Source: NIST Statistical Reference Datasets (StRD), Univariate Summary
Statistics collection, "Lower Level of Difficulty" dataset "Lew" (beam
deflection data, H. S. Lew, NIST Center for Building Technology):
https://www.itl.nist.gov/div898/strd/univ/data/Lew.dat

Fetched and cross-checked in-container on 2026-08-07: DATA below was
transcribed from that file's 200-observation data section, and CERTIFIED_MEAN /
CERTIFIED_STDEV were transcribed verbatim from its "Certified Values" header.
Recomputing mean/stdev over DATA reproduces both certified figures to within
1e-9 relative tolerance (see tests/test_smoke.py) -- the transcription is
self-verifying, not just copied by hand.
"""

DATASET_NAME = "Lew"

# 200 observations, NIST StRD Lew.dat lines 61-260.
DATA: list[float] = [
...
CERTIFIED_STDEV = 277.332168044316  # sample standard deviation, denom. = n-1
